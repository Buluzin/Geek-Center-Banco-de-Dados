drop database GEEK_CENTER;
	...
	CREATE SCHEMA IF NOT EXISTS GEEK_CENTER DEFAULT CHARACTER SET utf8;
	use GEEK_CENTER;
	...
	insert into Cliente  values ("Neymar ",'02598328127', "neymar@gmail.com", "QNA 23 casa 10");
	insert into Cliente  values ("Péricles",'02498328127', "Péricles@gmail.com", "QNA 23 casa 11");
	...
	insert into Produtos values('02', 2 , 'Camiseta do Omori', 55.00, 22 ,'PP-P-M-G-GG');
	insert into Produtos values('03', 2, 'Camiseta do Star Wars', 75.50, 15 ,'GG');
	insert into Produtos values('04', 1, 'Caneca do Attack on Titan', 115.90 , 50,'Único');
	insert into Produtos values('05', 2 , 'Camiseta do Harry Potter', 89.90 , 47 ,'PP-P-G');
	insert into Produtos values('06', 2 , 'Camiseta dos Jogos Vorazes', 35.00, 20 ,'PP-P-M-G-GG');
	insert into Produtos values('07', 2, 'Camiseta do Caça Fantasmas', 75.50, 15 ,'P-M-GG');
    insert into Produtos values('08', 1, 'Caneca do Tokyo Revengers', 15.97 , 10,'Único');
	insert into Produtos values('09', 1 , 'Caneca da Power', 45.70 , 80 ,'Único');
	insert into Produtos values('10', 2 , 'Camiseta do Sherlock Holmes', 100.00, 52 ,'PP-P-M-G-GG');
	insert into Produtos values('11', 2, 'Camiseta da Lisa Simpsons', 43.45, 26 ,'PP-P-M');
	...
	insert into Produtos values('18', 1 , 'Caneca de The Last of Us', 275.10, 22 ,'Único');
	insert into Produtos values('19', 2, 'Camiseta da Clementine ', 25.99, 187 ,'GG');
	insert into Produtos values('20', 1, 'Caneca do Life is Strange', 5.88 , 50,'Único');
    ...
    select max(preco_R$) as "Máximo do preço das canecas" from Produtos where categoria_id like 1 ;
    select min(preco_R$) as "Mínimo do preço das camisetas" from Produtos where categoria_id like 2;

SELECT Produtos.id_produto, encomenda_produto.produto_id 
FROM Produtos
INNER JOIN encomenda_produto ON Produtos.id_produto = encomenda_produto.produto_id;
 drop database GEEK_CENTER;